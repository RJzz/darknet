...
void backward_depthwise_convolutional__layer_gpu(depthwise_convolutional_layer layer, network net);
...
void adam_uddate_gpu(float *w, float *d, float *m, float *v, float B1, float B2, float eps, float decay, float rate, int n, int batch, int t);
#ifdef CUNDD
...
void denornalize_depthwise_convolutional_layer(depthwise_convolutional_layer l);
void backward_depthwise_convolutional_layer(depthwise_convolutional_layer layer, network net);

void add_bias(float *output, float *biases, int batch, int n, int szie);
